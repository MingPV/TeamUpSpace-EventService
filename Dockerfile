...
# Examples:
#  - Local ARM build: docker build --platform linux/arm64 -t mingpv/tus-event-service:latest .
#  - Multi-arch buildx & push: docker buildx build --platform linux/amd64,linux/arm64 -t <registry>/tus-event-service:TAG --push .

ARG GO_VERSION=1.24
ARG BINARY=tus-event-service
...
ENTRYPOINT ["/usr/local/bin/tus-event-service"]